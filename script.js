...
class GuessString {
  ...
  push(char) {
    if (this.guessChars.length < 5)
      this.guessChars = this.guessChars + char.toLowerCase();
  }

  pop() {
    if (this.guessChars.length == 0) {
      return;
    } else {
      this.guessChars = this.guessChars.slice(0, this.guessChars.length - 1);
      return;
    }
  }
...
class Pointer {
  ...
  gotoNextTile() {
    if (this.column < gameParameters.letterNumber - 1) {
      this.column++;
    }
    return;
  }
  ...
  gotoNextRow() {
    if (this.row < gameParameters.guessNumber - 1) {
      this.column = 0;
      this.row++;
    }
    return;
  ...
  getHtmlElement() {
    return document.getElementById(
      "row-index-" + this.row + "-column-index-" + this.column
    );
  }
  ...
  currentRowArray() {
    const currentRowContainer = document.getElementById(
      "row-index-" + this.row
    );
    const children = currentRowContainer.children;
...
const guessString = new GuessString();
const gameParameters = new GameParams();
...
createTooltipDiv("tooltip");
createTilesContainerDiv("letter-tiles");
createUserKeyboardDiv("user-keyboard");
createTiles();
createVirtualKeyboard();

function createHeader(gameTitle) {
...
function createTooltipDiv(divId) {
  const gameContainer = document.getElementById("game-container");
  const tooltipContainer = document.createElement("div");
  tooltipContainer.setAttribute("id", divId);
  tooltipContainer.classList.add("tooltip");
  tooltipContainer.textContent = "Doruk the Super!";
  gameContainer.append(tooltipContainer);
  return;
}
...
function createTilesContainerDiv(divId) {
  ...
  guessTiles.setAttribute("id", divId);
  guessTiles.classList.add("tiles-container");
  gameContainer.append(guessTiles);
  return;
}

function createUserKeyboardDiv(divId) {
  const gameContainer = document.getElementById("game-container");
  const userKeyboard = document.createElement("div");
  userKeyboard.setAttribute("id", divId);
  userKeyboard.classList.add("keyboard");
  gameContainer.append(userKeyboard);
  return;
}
...
function createTiles() {
  ...
  for (let rowIndex = 0; rowIndex < gameParameters.guessNumber; rowIndex++) {
    const tileRow = document.createElement("div");
    tileRow.setAttribute("id", "row-index-" + rowIndex);
    tiles.append(tileRow);
    for (
      let columnIndex = 0; columnIndex < gameParameters.letterNumber; columnIndex++
    ) {
...
function printLetter(key) {
  pointer.writeContent(key);
  guessString.push(key);
}
...
function processTooltip(message) {
...
function correctGuessAtTurn(lowerRowNumber, higherRowNumber=lowerRowNumber) {
  return (guessString.guessChars === secretWord && pointer.row >= lowerRowNumber && pointer.row <= higherRowNumber)
}
...
function processGuess() {
  ...
  else if (!DICT.includes(guessString.guessChars)) {
    invalidSubmission("Not in the dictionary!");
    return;
  }
  else if (pointer.row == (gameParameters.guessNumber - 1) && guessString.guessChars != secretWord) {
    colorGuessLetters();
    gameOver();
    return;
  }
  ...
  else if (correctGuessAtTurn(1, 4)) {
...
function processGuessLetters() {
  var word = secretWord;
  var guess = guessString.guessChars;
  var colors = Array(guess.length).fill(0);
  ...
  for (let i = 0; i < guess.length; i++) {
    if (colors[i] !== 2 && word.includes(guess[i])) {
      colors[i] = 1;
      word = word.replace(guess[i], " ");
    }
  }
...
function colorGuessLetters() {
  ...
  processedLetters.forEach((number, index) => {
    ...
    var squareValue = square.textContent.toUpperCase();
...
function animateTiles(style) {
  const divArray = pointer.currentRowArray();
...
function getTileFromCurrentRow(columnNumber) {
  return document.getElementById(
    "row-index-" + pointer.row + "-column-index-" + columnNumber
  );
}
...
function setTooltipTimer(duration = 1500) {
  setTimeout(() => {
    toggleTooltip();
  }, 1500);
  return;
